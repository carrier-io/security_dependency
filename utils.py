...
def run_test(test: SecurityDependencyTests, config_only=False) -> dict:
    engagement_id = None
    if test.integrations:
        engagement_id = test.integrations.get('reporters', {}).get('reporter_engagement')

    results = SecurityDependencyResults(
...
def parse_test_data(project_id: int, request_data: dict,
    ...
    try:
        test_data = rpc.call.security_dependency_test_create_common_parameters(
    ...
    for k, v in request_data.items():
        try:
            ...
            test_data.update(rpc.call_function_with_timeout(
                func=f'security_dependency_test_create_{k}',
                timeout=2,
                data=v,
