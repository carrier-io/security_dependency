...
class SecurityDependencyResults(db_tools.AbstractBaseMixin, db.Base, rpc_tools.RpcMixin):
    __tablename__ = "security_dependency_results"
    ...
    scan_type = Column(String(20), unique=False)
    ...
    def insert(self):
        self.test_config = SecurityDependencyTests.query.get(self.test_id).to_json()
        super().insert()
        # minio part
...
for i in [*SecurityReport.STATUS_CHOICES.keys(), *SecurityReport.SEVERITY_CHOICES.keys()]:
    setattr(SecurityDependencyResults, i, Column(Integer, unique=False, default=0))
