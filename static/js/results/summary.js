const updateSummary = async () => {
    ...
    const response = await fetch(`/api/v1/security_dependency/result/${getSelectedProjectId()}/${result_test_id}/`)
...
const reRunTest = () => {
    ...
    // const testId = result_test_id
    const result_test_id = new URLSearchParams(location.search).get('result_id')
    fetch(`/api/v1/security_dependency/rerun/${result_test_id}`, {
        method: 'POST'
    }).then(response => {
        if (response.ok) {
...
const scanners_formatters = () => {
    ...
    if (scanners.length>0){
        result = scanners.reduce(function(acc, curr){
            return acc + `<img class="mr-1" alt="${curr}" src="/security_dependency/static/assets/ico/${curr}.svg" width="20">`
        }, '')
