...
class API(Resource):
    ...
    def put(self, project_id: int, test_id: int):
        ...
        if isinstance(test_id, int):
            _filter = and_(
                SecurityDependencyResults.project_id == project_id, SecurityDependencyResults.id == test_id
            )
        else:
            _filter = and_(
                SecurityDependencyResults.project_id == project_id, SecurityDependencyResults.test_uid == test_id
            )
        test = SecurityDependencyResults.query.filter(_filter).first()
...
def write_test_run_logs_to_minio_bucket(test: SecurityDependencyResults, file_name='log.txt'):
